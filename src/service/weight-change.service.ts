...
import { createNewVersionFrom, createRequest, listRequests, findRequest } from "@/repository/weight-change.repository";
...
export async function approveRequest(args: {
    ...
    return sequelize.transaction(async (t) => {
        ...
        // marcar aprobada
        const  approve  = await import("@/dao/weight-change.dao"); // o reexporta en repo
        // mejor usa repository si prefieres: await WeightChangeDao.approve(...)
        const updated = await (await import("@/dao/weight-change.dao")).default.approve(req.id_weight_change_request, { transaction: t });
...
export async function rejectRequest(args: {
    ...
    return sequelize.transaction(async (t) => {
        ...
        const updated = await (await import("@/dao/weight-change.dao")).default.reject(req.id_weight_change_request, args.reason, { transaction: t });
...
export async function listVersions(modelId: number, filters: {
    ...
    return GraphVersionModel.findAll({ where, order: [["version_number_version", "DESC"]] });
}
