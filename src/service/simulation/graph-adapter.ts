// src/service/simulation/graph-adapter.ts
import Graph  from "node-dijkstra";

export interface IGraphAlgorithmAdapter {
    findPath(graph: any, from: string, to: string): { path: string[]; cost: number } | null;
}
...
export class NodeDijkstraAdapter implements IGraphAlgorithmAdapter {
    findPath(graph: any, from: string, to: string) {
        ...
        if (Array.isArray(route)) {
            return { path: route, cost: 0 }; // Asume un costo predeterminado si es un arreglo
        }
