...
async function bootstrap(): Promise<void> {
  try {
    ...
    logger.debug(
      "[bootstrap] Getting Sequelize instance using Database.getInstance()",
    );
    ...
    logger.debug(
      "[bootstrap] Testing database connection using Database.testConnection()",
    );
    ...
    app.listen(environment.apiPort, () => {
      logger.info(
        `[bootstrap] Server listening at http://localhost:${environment.apiPort}`,
      );
    });
  } catch (err: any) {
    logger.error({
