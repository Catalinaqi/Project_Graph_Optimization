...
export class BcryptPasswordHasher implements PasswordHasher {
    ...
    async hash(plain: string): Promise<string> {
        logger.debug("[BcryptPasswordHasher] Hashing password with bcrypt", { saltRounds: this.saltRounds });
        try {
            const hashed = await bcrypt.hash(plain, this.saltRounds);
            logger.info("[BcryptPasswordHasher] Password hashed successfully");
            return hashed;
        } catch (error: any) {
            logger.error("[BcryptPasswordHasher] Error hashing password", { error: error?.message });
            throw error;
        }
    }
    ...
    async compare(plain: string, hashed: string): Promise<boolean> {
        ...
        try {
            ...
            logger.info("[BcryptPasswordHasher] Password comparison result", { isMatch });
            return isMatch;
        } catch (error: any) {
            logger.error("[BcryptPasswordHasher] Error comparing passwords", { error: error?.message });
            throw error;
        }
    }
}
