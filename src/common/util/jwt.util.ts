...
export default class JwtUtils {
  ...
  private static readKey(
    ...
    if (!source || !source.trim()) {
      throw getError(
        ...
        `[JWT] ${label} key path/content not provided`,
      );
    }
    ...
    if (!fs.existsSync(abs)) {
      throw getError(
        ErrorEnum.SERVER_ERROR,
        `[JWT] ${label} key file not found at: ${abs}`,
      );
    }
    ...
    if (!pem) {
      throw getError(
        ErrorEnum.SERVER_ERROR,
        `[JWT] ${label} key file is empty at: ${abs}`,
      );
    }
    if (!pem.includes("BEGIN")) {
      throw getError(
        ErrorEnum.SERVER_ERROR,
        `[JWT] ${label} key at ${abs} is not a valid PEM`,
      );
    }
    logger.debug({
  ...
  private static readonly algorithm: Algorithm =
    (environment.jwtAlgorithm as Algorithm) || "RS256";
  private static readonly privateKey: string = JwtUtils.readKey(
    environment.jwtPrivateKeyPath,
    "private",
  );
  private static readonly publicKey: string = JwtUtils.readKey(
    environment.jwtPublicKeyPath,
    "public",
  );
  private static readonly expiresIn: number = Number(
    environment.jwtExpiresIn || 3600,
  );
    ...
    try {
      ...
      if (typeof decoded === "string") {
        ...
        throw getError(
          ...
          "JWT payload is a string, expected object",
        );
      }
