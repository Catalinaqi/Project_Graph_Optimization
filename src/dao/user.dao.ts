...
async function applyBalanceWithLock(
  ...
  const user = await GraphUserModel.findByPk(args.userId, {
    transaction: t,
    lock: t.LOCK.UPDATE,
  });
  if (!user) {
  ...
  if (affected !== 1) {
    const e: any = new Error("Balance update failed");
    e.status = 409;
    throw e;
  }
...
const UserDao: UserIdao = {
  async findByEmail(email: string, opt?: Tx): Promise<GraphUserModel | null> {
    ...
    try {
      return await GraphUserModel.findOne({
        where: { email_user: email },
        ...(opt || {}),
      });
    } catch (err: any) {
      logger.error(
        "[UserDao] Error in findByEmail email=%s err=%s",
        email,
        err?.message,
      );
      throw err;
  ...
  async findById(id: number, opt?: Tx): Promise<GraphUserModel | null> {
    ...
    try {
      ...
      logger.error(
        "[UserDao] Error in findById id=%s err=%s",
        id,
        err?.message,
      );
      throw err;
  ...
  async createUser(data: {
    ...
    try {
      ...
      logger.error(
        "[UserDao] Error in createUser email=%s err=%s",
        data.email_user,
        err?.message,
      );
      throw err;
  ...
  async setNewBalance(userId, rechargeTokens, performerId, reason, opt?: Tx) {
    ...
    const run = async (t: Transaction) => {
      const u = await GraphUserModel.findByPk(userId, {
        transaction: t,
        lock: t.LOCK.UPDATE,
      });
      if (!u) {
