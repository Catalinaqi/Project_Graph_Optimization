...
const WeightChangeDao: WeightChangeIdao = {
  ...
  findById(requestId, opt?: Tx) {
    ...
    return GraphWeightChangeRequestModel.findByPk(requestId, {
      ...(opt || {}),
    });
  },
  ...
  approve(requestId, opt?: Tx) {
    ...
    return GraphWeightChangeRequestModel.update(
      {
        status: GraphRequestStatusEnum.APPROVED,
        updated_at_request: new Date(),
      } as any,
      {
        where: {
          id_weight_change_request: requestId,
          status: GraphRequestStatusEnum.PENDING,
        },
