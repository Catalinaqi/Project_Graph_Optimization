...
export class GraphSimulationModel extends Model<
  InferAttributes<GraphSimulationModel>,
  InferCreationAttributes<GraphSimulationModel>
> {
  ...
  static initModel(sequelize: Sequelize): typeof GraphSimulationModel {
    logger.debug("[GraphSimulationModel] Initializing model...");

    GraphSimulationModel.init(
      {
        id_simulation: {
        ...
        start_weight_simulation: {
          type: DataTypes.DECIMAL(12, 4),
          allowNull: false,
        },
        end_weight_simulation: {
          type: DataTypes.DECIMAL(12, 4),
          allowNull: false,
        },
        step_weight_simulation: {
          type: DataTypes.DECIMAL(12, 4),
          allowNull: false,
        },
      ...
      },
      {
        ...
        modelName: "GraphSimulationModel",
        timestamps: false,
        indexes: [
          {
            fields: ["id_model", "created_at_simulation"],
            name: "idx_gs_model_created_at",
          },
        ],
      },
    );
    return GraphSimulationModel;
  }

  static associate(models: any) {
    logger.debug("[GraphSimulationModel] Registering associations...");

    GraphSimulationModel.belongsTo(models.GraphModel, {
      foreignKey: "id_model",
      as: "model",
      onDelete: "CASCADE",
    });
    GraphSimulationModel.belongsTo(models.GraphUser, {
      foreignKey: "id_user",
      as: "user",
      onDelete: "RESTRICT",
    });
    GraphSimulationModel.hasMany(models.GraphSimulationResult, {
